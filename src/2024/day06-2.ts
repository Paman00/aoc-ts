...
function getMapsInLoop(map: string[], start: number[]): number {
...
function isMapTravelLoop(
  ...
  const newMap = [...map];
  newMap[newObstacle[0]] =
    newMap[newObstacle[0]].substring(0, newObstacle[1]) +
    'O' +
    newMap[newObstacle[0]].substring(newObstacle[1] + 1);
  ...
  while (true) {
    ...
    if (!isInLimits(newMap, nextPos[0], nextPos[1])) break;

    const nextChar = newMap[nextPos[0]][nextPos[1]];
    if (nextChar === '.' || nextChar === '^') {
      pos = nextPos;
      const visitedValue = (visited.get(pos.toString()) ?? 0) + 1;
      visited.set(pos.toString(), visitedValue);
      if (visitedValue > 4) {
        return true;
      }
    } else {
