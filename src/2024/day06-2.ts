...
const visited = getVisitedMap(map, start);
const res = getMapsInLoop(map, start, visited);
console.log(res);
...
function getVisitedMap(map: string[], start: number[]): Set<string> {
...
function getMapsInLoop(map: string[], start: number[], visited: Set<string>) {
  let count = 0;
  visited.forEach((posVisited) => {
    const pos = posVisited.split(',').map(Number);
    if (isMapTravelLoop(map, start, pos)) {
      count++;
    }
  });
  return count;
...
function isMapTravelLoop(map: string[], start: number[], newObstacle: number[]): boolean {
  const newMap = [...map];
