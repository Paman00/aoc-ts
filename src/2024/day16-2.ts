...
function getUniqueTiles(
  ...
  let visitedTiles = new Set<string>();
  ...
  heap.insert({
    ...
    cost: 0,
    visited: new Set<string>([`${startX},${startY}`]),
  });
    ...
    for (let i = -1; i < 2; i++) {
      ...
      if (isValid(newX, newY, map)) {
        ...
        if (currentCost === undefined || newCost <= currentCost) {
          ...
          const newVisited = new Set<string>(current.visited);
